...
class ZeRO1Optimizer:
    def __init__(self, model: nn.Module, lr: float, world_size: int, rank: int):
        ...
        # NOTE: flat_param: 1D buffer with all params as views
        ...
        # NOTE: flat_grad: 1D buffer with all param.grad as views
        ...
        # NOTE: grad_chunk: gradient chunk for reduce-scatter (optimizer input)
        self.grad_chunk = torch.zeros(self.chunk_size, device=device, dtype=dtype)

        # NOTE: param_chunk: shard of flat_param for this rank (all-gather input)
...
if __name__ == "__main__":
    ...
    for step in range(num_steps):
        ...
        # NOTE: replacing optimizer.step() with ZeRO-1 step
        zero1_opt.step()
